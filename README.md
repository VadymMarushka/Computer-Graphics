...
| [**03. Fractals Visualizer**](./03_Fractals) | A WinUI 3 application that let's you construct some of algebraic and geometric fractals and then save them to a local SQLite gallery! Uses ComputeSharp, Escape Time Algorithm and Inigo Quilez gradient pallete formula for rendering and painting algebraic fractals. ScottPlot is used for drawing geometrical fractals. Fance additional features such as chromatic color pallete for algebraic fractals and evolution mode for geometrical fractals are implemented | C#, ComputeSharp, WinUI 3, MVVM, Entity Framework Core, SQLite |
